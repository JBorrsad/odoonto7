...
const createTestRunner = (PgPool, name) => {
    ...
    beforeEach(async (t) => {
        ...
        const pool0 = await (0, __1.createPool)('postgresql://' + POSTGRES_DSN, {
            maximumPoolSize: 1,
            PgPool,
        });
      ...
      `);
        ...
        await pool0.end();
        const pool1 = await (0, __1.createPool)(t.context.dsn, {
            maximumPoolSize: 1,
            PgPool,
        });
    ...
    afterEach(async (t) => {
        const pool = await (0, __1.createPool)('postgresql://' + POSTGRES_DSN, {
            maximumPoolSize: 1,
            PgPool,
        });
        try {
...
const createIntegrationTests = (test, PgPool) => {
    ...
    test.skip('transaction terminated while in an idle state is rejected (at the next transaction query)', async (t) => {
        ...
        await pool.connect(async (connection) => {
            await connection.query(__1.sql.unsafe `SET idle_in_transaction_session_timeout=500`);
            const error = await t.throwsAsync(connection.transaction(async (transaction) => {
                await (0, promises_1.setTimeout)(1000);
                await transaction.query(__1.sql.unsafe `SELECT 1`);
            }));
            t.true(error instanceof __1.BackendTerminatedError);
        });
        await pool.end();
    ...
    `);
        try {
            await pool.query(__1.sql.unsafe `SELECT * FROM error_notice(${10});`);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
        }
        catch (error) {
            if (error?.notices) {
